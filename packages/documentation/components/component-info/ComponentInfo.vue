	...
	<div class="component-info-container">
		...
		>
			<div style="display: flex; flex-wrap: wrap; gap: 4px">
				<template v-for="emit in component.emits" :key="emit">
					<code v-text="`@${emit}`" />
				</template>
			</div>
		</KtHeading>
...
import ComponentInfoSlots from './Slots.vue'
type Test = typeof ComponentInfoSlots.props

type PropOption = Exclude<Prop<unknown>, PropType<unknown>>
type VuePropType = PropOption['type']
		...
		return {
			...
			stringifyPropDefault: (
				...
				if (typeof defaultValue === 'function')
					return castArray(type).some(
						(t) => t && t !== true && t.name === 'Function',
					)
