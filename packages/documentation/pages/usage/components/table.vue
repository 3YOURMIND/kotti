...
export default defineComponent({
	...
	setup() {
		...
		const expandMode = ref<'multi' | 'single' | null>(null)
		const counter = ref(0)
		...
		const tableHook = useKottiTable<TableRow, ColumnId>(
			computed(() => ({
				...
				data: booleanFlags.value.isEmpty ? [] : data.value,
		...
		useLocalSort(data, tableHook.api.ordering, {
			...
			lifespan: (mode) => getTextSorter<TableRow>((row) => row.lifespan, mode),
