...
// 		if (isEqual(valueA, valueB)) return map.A_IS_B

// 		return isBefore(valueA, valueB) ? map.A_IS_SMALLER : map.B_IS_SMALLER
// 	}
// }
...
>(
	...
	const initialValue = clone(data.value)
	...
	watch(ordering, () => {
		...
		data.value = cloned.sort(sortingFunctions[sort.id](sort.value))
	})
}
