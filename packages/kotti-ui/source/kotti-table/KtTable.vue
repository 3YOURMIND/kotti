						...
						<td v-if="row.actions !== null" class="kt-table__actions-column">
							...
							>
								<i
									...
									@click="
										() => {
											row.isActionDisabled || action.onClick()
										}
									"
									v-text="action.icon"
								/>
		...
		return {
			bodyRows: computed(() =>
				table.value.getRowModel().rows.map((row) => {
					...
					const wrapComponent = getCellWrapComponent(behavior, row)
	...
	&__actions-column {
		...
		.kt-table__actions {
			...
			&--is-disabled {
				...
				border-color: rgb(224 224 224 / 60%);
			}
...
tr.kt-table-row {
	...
	&--is-interactive {
		...
		&:hover {
			background-color: rgb(234 240 250 / 50%);
		}
	}
